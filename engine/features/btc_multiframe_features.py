...
def _compute_vwap(prices: pd.DataFrame) -> pd.Series:
    """
    Daily VWAP (UTC) that *guarantees* reset by day.

    - Uses the DataFrame DateTimeIndex (preferred) and forces UTC.
    - Falls back to a timestamp column only if needed.
    - Uses typical price (H+L+C)/3 for VWAP calculation.
    """

    # --- Resolve UTC datetime series for grouping ---
    dt = None

    if isinstance(prices.index, pd.DatetimeIndex):
        idx = prices.index
        if idx.tz is None:
            dt = idx.tz_localize("UTC")
        else:
    ...
    if dt is None:
        ...
        for c in ["timestamp", "time", "datetime", "date"]:
            if c in prices.columns:
                dt = pd.to_datetime(prices[c], utc=True, errors="coerce")
                break

    if dt is None:
        raise ValueError("VWAP: cannot build datetime key (no DateTimeIndex and no timestamp column).")

    day_key = dt.floor("D")

    # --- VWAP calc (typical price) ---
    high = prices["high"].astype(float) if "high" in prices.columns else None
    low = prices["low"].astype(float) if "low" in prices.columns else None
    close = prices["close"].astype(float)

    if high is not None and low is not None:
        price_for_vwap = (high + low + close) / 3.0
    else:
        price_for_vwap = close  # fallback

    volume = prices["volume"].astype(float)
    vol_safe = volume.replace(0.0, np.nan)

    pv = price_for_vwap * vol_safe

    cum_pv = pv.groupby(day_key).cumsum()
    cum_vol = vol_safe.groupby(day_key).cumsum()

    vwap = cum_pv / cum_vol.replace(0.0, np.nan)
    vwap = vwap.ffill().fillna(close)
    return vwap
...
def _compute_vwap_dist_atr(
    ...
    dist_atr = dist_atr.clip(lower=-10.0, upper=10.0)
    return dist_atr
...
def build_btc_multiframe_features(prices: pd.DataFrame) -> pd.DataFrame:
    ...
    # VWAP distance (ATR units) + safety clamp
    df["vwap_dist_atr"] = _compute_vwap_dist_atr(df["close"], df["vwap"], df["atr"])
    df["vwap_dist_atr"] = df["vwap_dist_atr"].clip(lower=-10.0, upper=10.0)

    # VWAP distance (percent) — more stable for filtering/tuning
    df["vwap_dist_pct"] = (df["close"] - df["vwap"]) / df["close"].replace(0.0, np.nan)
    df["vwap_dist_abs_pct"] = (df["close"] - df["vwap"]).abs() / df["close"].replace(0.0, np.nan)
