from __future__ import annotations

"""
live/btc_po3_traderspost_loop.py

UNIFIED LIVE ENGINE (NO LLM YET):
---------------------------------
- Fetches BTC/USD 5m candles from TwelveData
- Writes them to data/5m_btc.csv (for institutional feature builder)
- Builds institutional multi-timeframe feature frame
- Runs BTCPO3PowerStrategy on the latest bar
- Sends LONG / SHORT signals to TradersPostExecutor (test mode by default)
- Tracks and prints live statistics each time a new bar is processed.
"""

import os
import sys
import time
import logging
from datetime import datetime, timezone, timedelta
from typing import Optional, Dict, Any

import pandas as pd

# ---------------------------------------------------------------------------
# 0. Ensure project root is on sys.path
# ---------------------------------------------------------------------------
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.abspath(os.path.join(CURRENT_DIR, os.pardir))

if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

# ---------------------------------------------------------------------------
# 1. Local imports
# ---------------------------------------------------------------------------
from config.config import load_config
from engine.data.twelvedata_btc import (
    fetch_btcusd_5m_history,
    fetch_latest_btcusd_5m_bar,
)
from engine.features.btc_multiframe_features import (
    build_btc_5m_multiframe_features_institutional,
)
from engine.strategies.smc_po3_power_btc import (
    BTCPO3PowerStrategy,
    BTCPO3PowerConfig,
)
from engine.execution.traderspost_executor import TradersPostExecutor

# ---------------------------------------------------------------------------
# 2. Paths & logging
# ---------------------------------------------------------------------------
DATA_DIR = os.path.join(PROJECT_ROOT, "data")
os.makedirs(DATA_DIR, exist_ok=True)

# This MUST match what build_btc_5m_multiframe_features_institutional() expects.
BTC_5M_CSV_PATH = os.path.join(DATA_DIR, "5m_btc.csv")

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
)


# ---------------------------------------------------------------------------
# 3. Helper: seconds until next 5m bar close (UTC)
# ---------------------------------------------------------------------------
def _seconds_until_next_5m(now: Optional[datetime] = None) -> int:
    """
    How many seconds until the next 5-minute boundary (UTC)?

    Bars close at hh:00, 05, 10, 15, ..., 55.
    We want to wake up just *after* the close, so we add +1s.
    """
    if now is None:
        now = datetime.now(timezone.utc)

    epoch = int(now.timestamp())
    rem = epoch % 300  # 300 seconds = 5 minutes
    wait = 300 - rem + 1
    return max(wait, 1)


# ---------------------------------------------------------------------------
# 4. Write prices -> 5m_btc.csv (for institutional builder)
# ---------------------------------------------------------------------------
def _write_prices_to_btc_5m_csv(prices: pd.DataFrame) -> None:
    """
    Save a prices DataFrame (index=datetime, OHLCV columns) to data/5m_btc.csv
    in the exact format the institutional feature builder expects.

    We also guarantee a valid numeric 'volume' column, because the institutional
    builder requires 'volume' and uses it to compute rvol_5m. If TwelveData
    returns missing / non-numeric volume, we synthesize a safe default.
    """
    df = prices.copy().reset_index()

    # Normalise timestamp column name
    if "datetime" in df.columns:
        df = df.rename(columns={"datetime": "timestamp"})
    else:
        # whatever the index name is, call it timestamp
        df = df.rename(columns={df.columns[0]: "timestamp"})

    # Ensure required OHLC columns exist
    for col in ["open", "high", "low", "close"]:
        if col not in df.columns:
            raise ValueError(f"Missing required column '{col}' in prices DataFrame.")

    # Handle 'volume'
    if "volume" not in df.columns:
        df["volume"] = 1.0
        logging.warning(
            "BTC_PO3_TRADER: No 'volume' column found in prices – "
            "synthesizing volume=1.0 for all rows."
        )
    else:
        df["volume"] = pd.to_numeric(df["volume"], errors="coerce")
        if df["volume"].isna().all():
            logging.warning(
                "BTC_PO3_TRADER: 'volume' column is all NaN after coercion – "
                "filling with 1.0 to allow rvol_5m computation."
            )
            df["volume"] = 1.0
        else:
            median_vol = df["volume"].median()
            if pd.isna(median_vol) or median_vol <= 0:
                median_vol = 1.0
            df["volume"] = df["volume"].fillna(median_vol)
            if (df["volume"] <= 0).all():
                logging.warning(
                    "BTC_PO3_TRADER: 'volume' column is non-positive – "
                    "replacing with 1.0 to allow rvol_5m computation."
                )
                df["volume"] = 1.0

    os.makedirs(os.path.dirname(BTC_5M_CSV_PATH), exist_ok=True)
    df.to_csv(BTC_5M_CSV_PATH, index=False)
    logging.info(
        "BTC_PO3_TRADER: Wrote %d rows to %s",
        len(df),
        BTC_5M_CSV_PATH,
    )


# ---------------------------------------------------------------------------
# 5. Bridge feature row -> strategy input dict
# ---------------------------------------------------------------------------
def _bridge_row_for_strategy(row: pd.Series) -> Dict[str, Any]:
    d = row.to_dict()

    close = float(d.get("close", float("nan")))
    atr14 = float(d.get("atr_14", float("nan")))
    rvol20 = float(d.get("rvol_20", float("nan")))
    week_pos = float(d.get("pos_in_week_range", float("nan")))
    vwap_dist_atr = float(d.get("vwap_dist_atr", float("nan")))

    session = d.get("session", None)
    regime_trend = d.get("regime_trend", None)  # "UP" / "DOWN" / None

    if close == close and atr14 == atr14 and close != 0.0:
        atr_pct_5m = atr14 / close
    else:
        atr_pct_5m = float("nan")

    regime_trend_up = 1 if str(regime_trend).upper() == "UP" else 0

    bridged = dict(d)
    bridged["atr_pct_5m"] = atr_pct_5m
    bridged["rvol_5m"] = rvol20
    bridged["week_pos"] = week_pos
    bridged["vwap_dist_atr"] = vwap_dist_atr
    bridged["session_type"] = session
    bridged["regime_trend_up"] = regime_trend_up

    return bridged


# ---------------------------------------------------------------------------
# 6. Strategy evaluation helper
# ---------------------------------------------------------------------------
def _evaluate_latest_bar(
    feat_df: pd.DataFrame,
    strategy: BTCPO3PowerStrategy,
) -> Dict[str, Any]:
    """
    Take the latest row of the feature frame and ask the strategy for a decision.
    Returns a dict summarising the decision and context.
    """
    if feat_df.empty:
        raise RuntimeError("Feature frame is empty – nothing to evaluate.")

    latest_idx = feat_df.index[-1]
    latest_row = feat_df.iloc[-1]

    row_dict = _bridge_row_for_strategy(latest_row)
    decision = strategy.on_bar(row_dict)

    result: Dict[str, Any] = {
        "timestamp": latest_idx,
        "raw_decision": decision,
        "row_dict": row_dict,
    }

    if isinstance(decision, str):
        result["side"] = decision
    elif isinstance(decision, dict):
        result["side"] = decision.get("side") or decision.get("action")
        result["context"] = {
            k: v for k, v in decision.items() if k not in ("side", "action")
        }
    else:
        result["side"] = str(decision)

    return result


# ---------------------------------------------------------------------------
# 7. Live stats helpers
# ---------------------------------------------------------------------------
def _init_live_stats() -> Dict[str, Any]:
    """Initialize a stats dict to track what's happening in the loop."""
    return {
        "start_time": datetime.now(timezone.utc),
        "bars_processed": 0,
        "signals": {"LONG": 0, "SHORT": 0, "HOLD": 0, "OTHER": 0},
        "orders_sent": 0,
        "last_signal": None,
        "last_signal_ts": None,
        "last_order": None,
        "last_order_ts": None,
    }


def _update_stats_on_signal(stats: Dict[str, Any], side: str, ts: Any) -> None:
    stats["bars_processed"] += 1
    key = side if side in ("LONG", "SHORT", "HOLD") else "OTHER"
    stats["signals"][key] += 1
    stats["last_signal"] = side
    stats["last_signal_ts"] = ts


def _update_stats_on_order(stats: Dict[str, Any], side: str, size: float, ts: Any, meta: Dict[str, Any]) -> None:
    stats["orders_sent"] += 1
    stats["last_order"] = {
        "side": side,
        "size": size,
        "meta": meta,
    }
    stats["last_order_ts"] = ts


def _format_timedelta(td: timedelta) -> str:
    total_seconds = int(td.total_seconds())
    hours = total_seconds // 3600
    minutes = (total_seconds % 3600) // 60
    seconds = total_seconds % 60
    return f"{hours:02d}:{minutes:02d}:{seconds:02d}"


def _print_live_stats(stats: Dict[str, Any]) -> None:
    """Print a clean, backtest-style summary block for the current live session."""
    now = datetime.now(timezone.utc)
    elapsed = now - stats["start_time"]

    bars = stats["bars_processed"]
    sigs = stats["signals"]
    orders = stats["orders_sent"]

    last_sig = stats["last_signal"]
    last_sig_ts = stats["last_signal_ts"]
    last_order = stats["last_order"]
    last_order_ts = stats["last_order_ts"]

    print("\n" + "=" * 55)
    print("==== BTC PO3 LIVE STATS (CURRENT SESSION) =================")
    print("=" * 55)
    print(f"Session started : {stats['start_time'].isoformat(timespec='seconds')}")
    print(f"Uptime          : {_format_timedelta(elapsed)}")
    print("-" * 55)
    print(f"Bars processed  : {bars}")
    print(f"Signals (LONG)  : {sigs['LONG']}")
    print(f"Signals (SHORT) : {sigs['SHORT']}")
    print(f"Signals (HOLD)  : {sigs['HOLD']}")
    print(f"Signals (OTHER) : {sigs['OTHER']}")
    print(f"Orders sent     : {orders}")
    print("-" * 55)

    if last_sig is not None:
        print(f"Last signal     : {last_sig} @ {last_sig_ts}")
    else:
        print("Last signal     : None yet")

    if last_order is not None:
        print(
            f"Last order      : {last_order['side']} {last_order['size']} "
            f"@ {last_order_ts} (test order payload sent)"
        )
    else:
        print("Last order      : None sent yet")

    print("=" * 55 + "\n")


# ---------------------------------------------------------------------------
# 8. Main live PO3 + TradersPost loop
# ---------------------------------------------------------------------------
def run_btc_po3_traderspost_loop() -> None:
    # 8A. Load config + wire TradersPost
    cfg = load_config()

    trade_symbol = getattr(cfg, "trade_symbol", "BTCUSD")
    feed_symbol = getattr(cfg, "feed_symbol", "BTCUSD")
    acct_size = getattr(cfg, "account_size", 0)
    live_trading = bool(getattr(cfg, "live_trading", False))
    default_size = float(getattr(cfg, "default_position_size", 0.01))

    logging.info(
        "BTC_PO3_TRADER: Loaded config | trade_symbol=%s | feed_symbol=%s | "
        "account_size=%s | live_trading=%s | default_size=%s",
        trade_symbol,
        feed_symbol,
        acct_size,
        live_trading,
        default_size,
    )

    executor = TradersPostExecutor(config=cfg)

    # 8B. Instantiate PO3 strategy with your tuned config
    strat_config = BTCPO3PowerConfig(
        min_atr_pct=0.0008,
        max_atr_pct=0.06,
        min_rvol=1.0,
        allow_asia=True,
        allow_london=True,
        allow_ny=True,
        use_trend_regime=True,
        min_week_pos_for_longs=0.20,
        max_week_pos_for_shorts=0.80,
        max_vwap_dist_atr_entry=2.0,
        require_sweep=False,
        lookback_sweep_bars=5,
        verbose=True,
    )
    strategy = BTCPO3PowerStrategy(config=strat_config)
    logging.info("BTC_PO3_TRADER: BTCPO3PowerStrategy instantiated.")

    # 8C. Init live stats
    stats = _init_live_stats()

    # 8D. Bootstrap recent BTC 5m prices from TwelveData
    logging.info(
        "BTC_PO3_TRADER: Bootstrapping BTCUSD 5m history from TwelveData (outputsize=2000)..."
    )

    prices = fetch_btcusd_5m_history(outputsize=2000)
    if prices is None or prices.empty:
        logging.error("BTC_PO3_TRADER: No history returned from TwelveData. Exiting.")
        return

    prices = prices.sort_index()
    logging.info(
        "BTC_PO3_TRADER: Bootstrapped %d 5m bars from %s to %s.",
        len(prices),
        prices.index[0],
        prices.index[-1],
    )

    # 8E. Write prices -> CSV and build institutional features
    _write_prices_to_btc_5m_csv(prices)

    feat_df, n_rows = build_btc_5m_multiframe_features_institutional()
    if feat_df.empty or n_rows == 0:
        logging.error(
            "BTC_PO3_TRADER: Institutional feature builder returned empty frame. Exiting."
        )
        return

    last_ts = feat_df.index[-1]
    logging.info(
        "BTC_PO3_TRADER: Feature frame built with %d rows (up to %s).",
        n_rows,
        last_ts,
    )

    # 8F. Evaluate latest bar once immediately
    initial_result = _evaluate_latest_bar(feat_df, strategy)
    side0 = initial_result.get("side")
    ts0 = initial_result.get("timestamp")

    _update_stats_on_signal(stats, side0, ts0)
    logging.info(
        "BTC_PO3_TRADER: Initial decision -> ts=%s | side=%s | raw=%r",
        ts0,
        side0,
        initial_result.get("raw_decision"),
    )

    _print_live_stats(stats)

    # 8G. Main loop
    logging.info("BTC_PO3_TRADER: Starting live 5m PO3 loop (TwelveData + TradersPost)...")

    while True:
        # 1) Sleep until next 5m boundary
        sleep_s = _seconds_until_next_5m()
        logging.info(
            "BTC_PO3_TRADER: Sleeping %d seconds until next 5m bar close...",
            sleep_s,
        )
        time.sleep(sleep_s)

        # 2) Fetch latest bar(s)
        try:
            latest_df = fetch_latest_btcusd_5m_bar()
        except Exception as exc:
            logging.warning("BTC_PO3_TRADER: Error fetching latest bar: %s", exc)
            continue

        if latest_df is None or latest_df.empty:
            logging.warning(
                "BTC_PO3_TRADER: No latest bar returned from TwelveData – skipping."
            )
            continue

        latest_df = latest_df.sort_index()
        latest_ts = latest_df.index[-1]

        if latest_ts <= last_ts:
            logging.info(
                "BTC_PO3_TRADER: No new 5m bar yet (latest=%s, last_seen=%s).",
                latest_ts,
                last_ts,
            )
            continue

        new_prices = latest_df[latest_df.index > last_ts]
        if new_prices.empty:
            continue

        # 3) Update in-memory prices and rewrite CSV
        prices = pd.concat([prices, new_prices]).sort_index()
        _write_prices_to_btc_5m_csv(prices)

        # 4) Rebuild institutional features from CSV
        feat_df, n_rows = build_btc_5m_multiframe_features_institutional()
        if feat_df.empty or n_rows == 0:
            logging.warning(
                "BTC_PO3_TRADER: Institutional feature builder returned empty frame after update – skipping."
            )
            continue

        last_ts = feat_df.index[-1]
        latest_row = feat_df.iloc[-1]

        # 5) Evaluate strategy on latest bar
        result = _evaluate_latest_bar(feat_df, strategy)
        side = result.get("side")
        raw_decision = result.get("raw_decision")
        ts = result.get("timestamp")
        row_dict = result.get("row_dict", {})

        close = float(row_dict.get("close", float("nan")))
        atr_pct = float(row_dict.get("atr_pct_5m", float("nan")))
        rvol = float(row_dict.get("rvol_5m", float("nan")))
        week_pos = float(row_dict.get("week_pos", float("nan")))
        vwap_dist_atr = float(row_dict.get("vwap_dist_atr", float("nan")))
        session = row_dict.get("session_type", None)
        regime_up = row_dict.get("regime_trend_up", None)

        logging.info(
            "BTC_PO3_TRADER: [LIVE] ts=%s | close=%.2f | atr_pct=%.4f | "
            "rvol=%.2f | week_pos=%.2f | vwap_dist_atr=%.2f | "
            "session=%s | trend_up=%s | side=%s | raw=%r",
            ts,
            close,
            atr_pct,
            rvol,
            week_pos,
            vwap_dist_atr,
            session,
            regime_up,
            side,
            raw_decision,
        )

        # Update stats on signal
        _update_stats_on_signal(stats, side, ts)

        # 6) Only act on LONG / SHORT
        if side not in ("LONG", "SHORT"):
            _print_live_stats(stats)
            continue

        # 7) Build meta and send order
        now_utc = datetime.now(timezone.utc).isoformat(timespec="seconds")
        test_flag = True  # keep True for now; flip when fully confident

        meta: Dict[str, Any] = {
            "source": "btc_po3_traderspost_loop",
            "trade_symbol": trade_symbol,
            "feed_symbol": feed_symbol,
            "generated_at": now_utc,
            "bar_timestamp": ts.isoformat() if isinstance(ts, datetime) else str(ts),
        }

        logging.info(
            "BTC_PO3_TRADER: Sending %s size=%s to TradersPost (test=%s, live_trading=%s)...",
            side,
            default_size,
            test_flag,
            live_trading,
        )

        try:
            executor.send_order(side, default_size, test_flag, meta)
            _update_stats_on_order(stats, side, default_size, ts, meta)
        except Exception as exc:
            logging.error("BTC_PO3_TRADER: Error sending order to TradersPost: %s", exc)

        # Print stats after each order
        _print_live_stats(stats)


# ---------------------------------------------------------------------------
# 9. Entrypoint
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    run_btc_po3_traderspost_loop()
